import { ArrowUpRight, Bell, ClipboardList, FlaskConical, LayoutDashboard, LogOut, Search, Settings, Sprout, Users, CheckCircle2, Filter, Layers3 } from "lucide-react";

const sidebarItems = [
    { label: "Dashboard", icon: LayoutDashboard, active: true },
    { label: "Permohonan", icon: ClipboardList, soon: true },
    { label: "Laboratorium", icon: FlaskConical, soon: true },
    { label: "Benih", icon: Sprout, soon: true },
    { label: "User", icon: Users, soon: true },
    { label: "Pengaturan", icon: Settings, soon: true },
];
                ...
                <div className="flex h-full flex-col gap-6 p-5">
                    ...
                    <nav className="flex flex-1 flex-col gap-2">
                        {sidebarItems.map((item) => {
                            ...
                            return (
                                <button key={item.label} className={["flex items-center justify-between gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium transition", item.active ? "bg-brand-500 text-white shadow-glow" : "text-white/75 hover:bg-white/10 hover:text-white"].join(" ")}>
                                    <span className="flex items-center gap-3">
                                        <Icon size={18} />
                                        <span>{item.label}</span>
                                    </span>
                                    {item.soon ? <span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.2em]">Soon</span> : null}
                                </button>
                            );
                        })}
                    </nav>
